...
for tmpfile in files:
  ...
  if not os.path.isfile(specfile):
    ...
    # Define the Model
    AllModels += "const*tbabs*nthcomp"
    m1 = AllModels(1)

    m1(1).values = "1 -1"    	# Constant
    m1(2).values = "3 -1"	# nH
    m1(3).values = "1.5 1"	# Gamma
    m1(4).values = "80. -1"	# kTe
    m1(5).values = "0.1 -1"	# kTbb
    m1(6).values = "1 -1"	# inp_type
    m1(7).values = "0 -1"	# z
    m1(8).values = "1. 1"	# Normalization
    ...
    # Calculate Errors
    Fit.error("maximum 200. 2.706 3 8")
    ...
    norms.append([m1(8).values[0],m1(8).error[0],m1(8).error[1]])
...
# Define the Model
m1 = Model("tbabs*nthcomp")

m1(1).values = "3. -1"			# nH
m1(2).values = str(avgGamma)+" 1"	# Gamma
m1(3).values = "80. -1"     		# kTe
m1(4).values = "0.1 -1"     		# kTbb
m1(5).values = "1 -1"       		# inp_type
m1(6).values = "0 -1"       		# z
m1(7).values = str(avgNorm)+" 1"	# Normalization
